...
if model_type == 'Linear':
    ...
    #for i in range(nClusters):
        #eval_results_shapley['eval{0}'.format(i)]['test']['rmse'] = min(np.sqrt(np.mean((model_dict_shapley['model{0}'.format(i)].predict(xgboost.DMatrix(original_split_shapley['original_test_cluster{0}'.format(i)])).reshape(-1,1) - original_split_shapley['original_test_label_cluster{0}'.format(i)])**2)))
...
if model_type == 'Linear':
    model_tot = LinearRegression().fit(X_train,y_train)
    ev_result ={'test': {'rmse': cluster._calculate_accuracy(model_tot.predict,X_test,y_test)}}
    tot_result_big_model = ev_result['test']['rmse']
else:
    model_tot = xgboost.train(params,xgboost.DMatrix(X_train, label=y_train),10000,evals = [(xgboost.DMatrix(X_train, label=y_train), "train"),(xgboost.DMatrix(X_test, label=y_test), "test")] ,verbose_eval = 1000,evals_result = ev_result,early_stopping_rounds = 200)
